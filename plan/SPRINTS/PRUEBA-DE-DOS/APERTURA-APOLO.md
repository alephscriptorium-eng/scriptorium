...
| b | **Spike `operador-rooms`**: valorar skill para el operador-admin de un nodo rooms — emisión de peercard vía `issuePeerCard`/`onPeerCard` de `@zeus/authority-kit`, ACL, salud del nodo | PD-02 de este sprint · fuente `playground/prueba-de-dos/reference/PEERCARD.md` | **decisión custodio PENDIENTE** — solo valoración, no implementar sin GO |
| c | **Intake del skill `prueba-de-dos`**: valorar su paso de boilerplate del workspace (`playground/prueba-de-dos/`) a skill del paquete en la librería | PD-01 de este sprint | **decisión custodio PENDIENTE** — solo valoración |
