...
| ✎ 2026-08-02 | **Hotfix LORE-HM CI**: tip `wp/lore-hm-accum` `2c4abea` estaba **CI rojo medido** (run `30734537647`) — `test-100-schemas` exigía `length===11` con 14 schemas legítimos post-109. Arreglo: guardian por **presencia de los once nombrados** (no cardinalidad). Latente post-fix: sellos Onfalo/mapa con bytes CRLF en git (`-text`) para que el runner Linux no rompa sha256. **NO-GO merge** hasta custodio; run-id verde post-fix se anota al cerrar. |
